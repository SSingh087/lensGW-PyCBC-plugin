...
if __name__ == "__main__":
    ...
    hp_tilde_lensed, hc_tilde_lensed = waveform.get_td_waveform(
                    ...
                    distance=1000,coa_phase=2.45,delta_t=1.0/16384,f_lower=20)
    pylab.figure(figsize=(18,10))
    pylab.xticks(fontsize=18)
    pylab.yticks(fontsize=18)
    pylab.plot(hp_tilde_lensed.sample_times, hp_tilde_lensed,label='h$_+$')
    pylab.plot(hp_tilde_lensed.sample_times, hc_tilde_lensed,label='h$_x$')
    pylab.legend(loc=0,fontsize=18)
    plt.tight_layout()
